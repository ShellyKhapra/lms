...
public class SpringBundle<T extends Configuration> implements ConfiguredBundle<T> {
    ...
    /**
     * Register {@link ServerLifecycleListener}s in Dropwizard {@link Environment} from Spring
     * application context.
     *
     * @param environment the Dropwizard environment
     * @param context the Spring application context
    ...
    private void registerServerLifecycleListeners(
        ...
        Map<String, ServerLifecycleListener> beansOfType =
        ...
        for (String beanName : beansOfType.keySet()) {
            ...
            if (!beanName.equals(ENVIRONMENT_BEAN_NAME)) {
                ...
                LOG.info(
                        "Registering serverLifecycleListener: " + serverLifecycleListener.getClass().getName());
            }
        }
    }
    ...
    /**
     * Register {@link HealthCheck}s in Dropwizard {@link Environment} from Spring application
     * context.
     *
     * @param environment the Dropwizard environment
     * @param context the Spring application context
    ...
    /**
     * Register {@link InjectionResolver}s in Dropwizard {@link Environment} from Spring application
     * context.
     *
     * @param environment the Dropwizard environment
     * @param context the Spring application context
    ...
    private void registerInjectableProviders(
            Environment environment, ConfigurableApplicationContext context) {
        final Map<String, InjectionResolver> beansOfType =
                context.getBeansOfType(InjectionResolver.class);
        for (String beanName : beansOfType.keySet()) {
            // Add injectableProvider to Dropwizard environment
            InjectionResolver injectableProvider = beansOfType.get(beanName);
    ...
    /**
     * Register resources annotated with {@link Path} in Dropwizard {@link Environment} from Spring
     * application context.
     *
     * @param environment the Dropwizard environment
     * @param context the Spring application context
